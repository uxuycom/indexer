...
import (
	...
	"math/big"
	"net/url"
	"open-indexer/client/btc/rpcclient"
	"os"
	"time"
)
...
func (c *RawClient) GetMultiRawTransactionVerboseErrors(ctx context.Context, reqHashes []string) (txs map[string]*btcjson.TxRawResult, errors map[string]error, err error) {
	if len(reqHashes) <= 0 {
		return nil, nil, nil
	}
	...
	for {
		...
		for _, hash := range sliceHashes {
			...
			if err1 != nil {
				errors[hash] = fmt.Errorf("decode hash[%s] err[%v]", hash, err1)
				continue
			}
		...
		if err = c.batchClient.Send(); err != nil {
			return nil, nil, fmt.Errorf("batch request error[%v], hashes[%v]", err, batch)
		}

		for hash, future := range futures {
			tx, err1 := future.Receive()
			if err1 != nil {
				errors[hash] = fmt.Errorf("receive tx hash[%s] error[%v]", hash, err1)
				continue
			}
			txs[hash] = tx
		}
		...
		// fix request frequency limit
		<-time.After(time.Second)
	}
	return txs, errors, nil
}
