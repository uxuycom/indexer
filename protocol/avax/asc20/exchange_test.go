		...
		t.Run(test.Name, func(t *testing.T) {
			...
			if len(result) == len(test.Expected) {
				for idx, item := range result {
					assert.Equal(t, item.Tick, result[idx].Tick)
					assert.Equal(t, item.From, result[idx].From)
					assert.Equal(t, item.To, result[idx].To)
					assert.Equal(t, item.Amount, result[idx].Amount)
				}
			}
		})
	}
			...
			Expected: []*Exchange{
				{
					...
					Amount: decimal.NewFromInt(123456),
				},
			},
		},
	}
