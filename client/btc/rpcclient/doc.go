...
There are 3 categories of xyerrors that will be returned throughout this package:
  ...
  - Errors that occur before communicating with the remote RPC server such as
    command creation and marshaling xyerrors or issues talking to the remote
    server
...
The first category of xyerrors are typically one of ErrInvalidAuth,
...
The second category of xyerrors typically indicates a programmer error and as such
the type can vary, but usually will be best handled by simply showing/logging
it.

The third category of xyerrors, that is xyerrors returned by the server, can be
...
detect if a command is unimplemented by the remote RPC server:
	  ...
	  if err != nil {
	  	if jerr, ok := err.(*btcjson.RPCError); ok {
	  		...
	  		// Handle other specific xyerrors you care about
			}
	  	}
