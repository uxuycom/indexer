...
func (c *Client) handleSendPostMessage(jReq *jsonRequest) {
	...
	for i := 0; i < tries; i++ {
		...
		log.Debugf("JSONRPC-CALL, method[%s], cost[%v], err[%v]", jReq.marshalledJSON, time.Since(start), err)
	...
	var res []byte
	if c.batch {
		// xyerrors must be dealt with downstream since a whole request cannot
...
// to easily return xyerrors from the various Async functions.
...
// reply or any xyerrors.  The examined xyerrors include an error in the
...
func (c *Client) Disconnect() {
	...
	// When operating without auto reconnect, send xyerrors to any pending
...
// NewBatch is a factory that creates a client able to interact with the server using
